...
By default the gateway does nothing about downstream DNS — clients must bring their own resolver and their queries route out the tunnel like any other traffic. `GATEWAY_DNS` (ported from the sibling [nordvpn](https://github.com/azinchen/nordvpn) and [openconnect-client](https://github.com/azinchen/openconnect-client) images) intercepts port-53 traffic from the `FORWARD_FROM` CIDRs instead:

- **`redirect`** — client DNS is DNAT-ed to the first IPv4 resolver from the [`DNS` variable](Custom-DNS) (NordVPN's own resolvers by default) and travels through the tunnel. Unlike the OpenVPN-based siblings the resolvers are static — WireGuard servers push nothing — so the rules are set once at startup and survive reconnects unchanged. Clients with hardcoded resolvers are captured too; queries aimed at an address local to the gateway's own netns pass untouched, so a co-located resolver stays reachable.
...
- **`forward`** — **all** client port-53 traffic is DNAT-ed to `GATEWAY_DNS_SERVER`, reached **directly over the uplink, not through the tunnel** — e.g. an AdGuard Home on your LAN. The gateway pins a host route and masquerades the hairpinned queries so this keeps working while the tunnel owns the default route. `GATEWAY_DNS_SERVER` may list several IPv4 resolvers (`;`/`,`-separated); at startup each is probed with a DNS query and the first one that answers is used, so a listed backup resolver takes over automatically when the primary is down at boot.
